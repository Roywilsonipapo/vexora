...
export default class ClientStore {
    ...
    onAuthorizeEvent = (data: {
        ...
        if (data?.current_account) {
            ...
            this.subscribeToLiveBalance();
        }
    };

    balance_message_subscription: { unsubscribe: () => void } | null = null;

    subscribeToLiveBalance = () => {
        if (!api_base?.api) return;
        // Avoid double subscriptions if authorize fires more than once.
        this.balance_message_subscription?.unsubscribe();

        api_base.api.send({ balance: 1, subscribe: 1 }).catch(() => {});

        this.balance_message_subscription = api_base.api.onMessage().subscribe(({ data }: { data: { msg_type: string; balance?: { balance: number; currency: string } } }) => {
            if (data?.msg_type === 'balance' && data.balance) {
                this.setBalance(String(data.balance.balance));
                if (data.balance.currency) this.setCurrency(data.balance.currency);
            }
        });
    };
