...
const AppWrapper = observer(() => {
    ...
    // Cross-app "Load Bot" support: if opened as ?loadBot=<filename>, fetch that
    // file from /free-bots/ and load it straight into Bot Builder automatically.
    // This is what lets Digits' Free Bots tab send people here pre-loaded.
    React.useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        const file = params.get('loadBot');
        if (!file) return;
        ...
        (async () => {
            try {
                ...
                // Silent — user can still load it manually via My computer.
            }
        })();
