...
export function Header({
  ...
  const liveBalance = useLiveBalance(
    activeAccount ? { balance: Number(activeAccount.balance), currency: activeAccount.currency } : null
  );
              ...
              <button className="flex items-center gap-2 rounded-lg border border-border px-3 hover:bg-muted/50 transition-colors">
                <div className="text-left">
                  ...
                  <p className="text-base font-bold text-foreground">
                    {liveBalance ? formatBalance(String(liveBalance.balance)) : formatBalance(activeAccount.balance)}{' '}
                    {liveBalance?.currency ?? activeAccount.currency}
                  </p>
                </div>
                <svg
                  className={cn(
